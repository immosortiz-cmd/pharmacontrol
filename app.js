...
function loadView(v){
  ({
    ...
    onedrive:odInitPanel, apariencia:loadApariencia
  })[v]?.();
}
...
function loadBackups(){
  ...
  $('backups-tbody').innerHTML=filtered.slice().reverse().map(b=>`<tr><td><strong>${esc(b.maquinaNombre||'—')}</strong></td><td>${esc(b.dispositivoModelo||'—')}</td><td>${tipoBadge(b.dispositivoTipo||'—')}</td><td style="font-family:var(--mono);font-size:11px">${esc(b.version||'—')}</td><td>${esc(b.dispositivoAlmacenamiento||'—')}</td><td style="font-family:var(--mono);font-size:11px;color:var(--text3)">${esc(b.rutaUbicacion||'—')}</td><td style="font-family:var(--mono);font-size:11px">${fmtDT(b.fecha)}</td><td>${esc(b.responsable||'—')}</td><td style="font-size:11px;color:var(--text3)">${esc(b.notas||'')}</td><td><button class="btn btn-sm btn-danger" onclick="deleteBackup('${b.id}')"><i class="ti ti-trash"></i></button></td></tr>`).join('');
}
...
function newBackupModal(){
  ...
  ()=>{
    ...
    const r=DB.addBackup({maquinaId:mId,maquinaNombre:m.nombre,dispositivoId:dId,dispositivoModelo:d.modelo,dispositivoTipo:d.tipo,version:fv('bk-ver'),responsable:fv('bk-resp'),fecha:fv('bk-fecha')?new Date(fv('bk-fecha')).toISOString():new Date().toISOString(),dispositivoAlmacenamiento:fv('bk-alm'),rutaUbicacion:fv('bk-ruta'),notas:fv('bk-notas'),frecuenciaDias:frecDias});
    closeModal();
